...
public class Message implements Serializable {
	...
	public enum Verb {ADDNODE, UPDLIST, QUIT, CHECKNODE, FIND, GETDOC, NEWDOC, REPLICATEDOC }
	public enum Answer {OK, NOK, NO, YES };
	
	protected int id;
	/**
	 * Answer or answer waited:
	 * in an answer, either OK or NOK (not OK)
	 * in a question, either YES if an answer is requested, or NO if no answer is requested
	 */
	...
	private Message(MessageNodeAddress source, Subject subject, Verb verb, Answer answer) {
	...
	/**
	 * Change the answer value (if it's a question, do we wait for an answer, if it's an answer, is it ok or not)
	 * @param answer
	 */
	...
	public static Message addNode(MessageNodeAddress source) {
		Message m = new Message(source, Subject.CONNECTION, Verb.ADDNODE, Answer.YES);
		return m;
	}
	public static Message okAddNode(int messageId, MessageNodeAddress source, MessageNodeAddress[] list, String domainName) {
		Message m = new Message(source, Subject.CONNECTION, Verb.ADDNODE, Answer.OK);
		m.id = messageId;
		m.domainListNode = list;
		m.domainName = domainName;
		return m;
	}
	public static Message updList(MessageNodeAddress source, MessageNodeAddress[] list) {
		Message m = new Message(source, Subject.CONNECTION, Verb.UPDLIST, Answer.NO);
		m.domainListNode = list;
		return m;
	}
	public static Message quit(MessageNodeAddress source, MessageRessource[] ressourceList ) {
		Message m = new Message(source, Subject.CONNECTION, Verb.QUIT, Answer.NO);
	...
	public static Message checkNode(MessageNodeAddress source) {
		Message m = new Message(source, Subject.CONNECTION, Verb.CHECKNODE, Answer.YES);
		return m;
	}
	public static Message okCheckNode(int messageId, MessageNodeAddress source) {
		Message m  = new Message(source, Subject.CONNECTION, Verb.CHECKNODE, Answer.OK);
		m.id = messageId;
		return m;
	}
	//ADDNODE, UPDLIST, QUIT, CHECKNODE, FIND, GETDOC, NEWDOC, REPLICATEDOC
	public static Message find(MessageNodeAddress source, MessageRessource ressource) {
		Message m = new Message(source, Subject.REPLICATION, Verb.FIND, Answer.YES);
	 ...
	 * @param source the local node address
	 * @param receivedMessage the received message that was expecting to a response but doesn't have one
	 * @return
	 */
	public static Message noOk(MessageNodeAddress source, Message receivedMessage) {
		Message m = new Message(source, receivedMessage.getSubject(), receivedMessage.getVerb(), Answer.NOK);
		m.id = receivedMessage.getId();
		return m;
	}
	
	/**
	 * Is the message an answer ? If not, it's a question, waiting (or not) for an answer
	 * @return
	 */
